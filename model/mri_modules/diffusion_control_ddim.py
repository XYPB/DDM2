...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, canny, t, clip_denoised: bool, mask=None, condition_x=None, 
        ...
        if ttt_opt is None:
            with torch.no_grad():
                eps_t = flip_canny_denoise(x, canny, self.denoisor, single_noise_level.expand(4, -1), 
                                      flips = [(False, False), (True, False), (False, True), (True, True)])
                x_recon = self.predict_start_from_noise(x, t, eps_t)
        else:
            # TTT
            ttt_opt.zero_grad()
        ...
        return x_recon, eps_t

    @torch.no_grad()
    def p_sample(self, x, canny, t, t_prev, clip_denoised=True, condition_x=None, mask_condition=None, ttt_opt=None):
        x_recon, eps_t = self.p_mean_variance(
            x=x, canny=canny, t=t, clip_denoised=clip_denoised, condition_x=condition_x, mask_condition=mask_condition, ttt_opt=ttt_opt)
    ...
    def p_sample_loop(self, x_in, continous=False, ttt_opt=None, matched_state=1000):
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_prev)), desc='sampling loop time step', total=len(seq)):
            ...
            # ob, c, w, h = img.shape
            img, noise, eps_t, x_recon = self.p_sample(img, canny, i, j, self.eta, condition_x=x_recon, ttt_opt=ttt)
            ttt = ttt_opt
    ...
    def p_losses(self, x_in, noise=None, debug=False):
        ...
        for t_ in t:
            continuous_sqrt_alpha_cumprod.append(np.random.uniform(
        ...
        eps_t = self.denoisor(x_noisy, canny, continuous_sqrt_alpha_cumprod)
