...
class MLPFeatureWiseAffine(nn.Module):
    def __init__(self, in_channels, out_channels, use_affine_level=False):
        super(MLPFeatureWiseAffine, self).__init__()
    ...
    def forward(self, x, noise_embed):
        ...
        if self.use_affine_level:
            ...
            noise = self.noise_func(noise_embed)
            #@print(noise.shape, x.shape, noise_embed.shape, '??')
            noise = noise.expand(batch, -1)#.view(batch, -1)
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=32, dropout=0, additional_dim=0):
        super().__init__()
        ...
        self.conv = nn.Conv2d(dim+additional_dim, dim_out, 3, padding=1)
